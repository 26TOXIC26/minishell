...
/*   Updated: 2024/08/29 20:08:28 by bamssaye         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_wordcount2(char const *s, int *i, int *count)
{
	while (s[*i])
	{
		if (s[*i] && w_space(s[*i]))
...
static void	set_word2(char *word, char const *s, int *i, int *j)
{
	word[*j] = s[*i];
	(*j)++;
	(*i)++;
